...
export default function Hero() {
  return (
    <section className="relative min-h-[100svh] flex flex-col bg-bg overflow-hidden">
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at top right, var(--glow-sage) 0%, transparent 70%)",
        }}
      />
      <div
        aria-hidden="true"
        ...
        style={{
          background:
            "radial-gradient(ellipse at bottom left, var(--glow-peri) 0%, transparent 70%)",
        }}
      />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-10 lg:gap-16 items-end">
          <div>
            ...
            <div className="flex items-center gap-5 mb-10">
              ...
              />
              <div>
                <p className="font-mono text-[10px] tracking-[0.22em] uppercase text-sage mb-1">
                  {hero.eyebrow}
                </p>
                <p className="flex items-center gap-3">
                  <span className="w-10 h-px bg-sage/50" aria-hidden="true" />
            ...
            <h1 className="mb-8">
              ...
              <span aria-hidden="true" className="block">
                <span
                  className="font-display font-[700] leading-[0.88] tracking-[-0.03em] text-fg block"
                ...
                <span className="flex items-end gap-5 md:gap-8">
                  <span
                    className="font-display font-[700] leading-[0.88] tracking-[-0.03em] text-fg"
                    style={{ fontSize: "clamp(76px, 15vw, 220px)" }}
                  >
                    {hero.headlineTail}
                  </span>
                  <span className="pb-3 md:pb-5 flex flex-col">
                    <span
                      className="font-display font-[300] italic text-sage leading-none"
                      style={{ fontSize: "clamp(30px, 5.5vw, 80px)" }}
                    >
                      {hero.year}
                    </span>
                    <span className="font-mono text-[10px] tracking-[0.18em] uppercase text-faint mt-1">
                      {hero.yearPlace}
                    </span>
                  </span>
                </span>
            ...
            <p className="font-sans text-[16px] md:text-[18px] text-muted leading-[1.65] max-w-[520px] mb-12">
              {hero.subtitle}
            </p>
            ...
            <div id="register" className="flex flex-wrap items-center gap-4 scroll-mt-24">
              <a
                href={links.register}
                className="font-sans text-[12px] tracking-[0.14em] uppercase px-8 py-4 bg-sage text-on-accent font-[700] hover:bg-sage-bright transition-colors duration-200"
              >
                {hero.ctaPrimary}
              </a>
              <a
                href="#events"
                className="font-sans text-[12px] tracking-[0.14em] uppercase px-8 py-4 border border-line-strong text-fg hover:border-line-stronger transition-colors duration-200"
              >
                {hero.ctaSecondary}
              </a>
            </div>
          ...
          {/* Event details */}
          <dl className="border border-line bg-surface backdrop-blur-sm divide-y divide-line">
            {hero.details.map((row) => (
              <div key={row.label} className="px-6 py-4">
                <dt className="font-mono text-[10px] tracking-[0.2em] uppercase text-faint mb-1">
                  {row.label}
                </dt>
                <dd className="font-sans text-[15px] text-fg">{row.value}</dd>
              </div>
            ))}
          </dl>
        </div>
